...
const getIconByName = (name) => iconMap[name] || faFile;
    ...
    fetch("http://localhost:8000/api/categories")
      ...
      .then(data => {
        const mapped = data.map(cat => ({
          ...cat,
          icon: getIconByName(cat.icon || 'faFile')
        }));
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDocumentCategories(prev => [
          ...prev,
          { ...added, icon: getIconByName(added.icon) }
        ]);
        setShowAddCategoryModal(false);
        alert("已成功新增類別");
      } else if (res.status === 409) {
        alert("類別 ID 已存在，請使用其他識別碼");
      } else {
        alert("新增類別失敗，請稍後再試");
      }
    } catch (err) {
      console.error("新增類別時發生錯誤", err);
      alert("發生錯誤，請確認後端服務是否正常");
    }
  };
